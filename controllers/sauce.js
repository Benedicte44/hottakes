...
exports.getAllSauces = (req, res, next) => {
	// the function to get all sauces
...
exports.modifySauce = (req, res, next) => {
	// function to modify the datas of a sauce
...
exports.likeDislikeSauce = (req, res, next) => {
	Sauce.findOne({ _id: req.params.id })
		.then((sauce) => {
			if (
				req.body.like == 1 &&
				sauce.usersLiked.includes(req.auth.userId) == false &&
				sauce.usersDisliked.includes(req.auth.userId) == false
			) {
				Sauce.updateOne(
					{ _id: req.params.id },
				...
				if (
					...
					res
						.status(400)
						.json({ message: "Vous avez déjà revendiqué votre avis !" });
				}
			}
		})
		.catch((error) => res.status(400).json({ error }));
